  ...
  try {
    ...
    bot.api
      .setMyShortDescription(
        "SwiftyEx Mini App — Yarn (AI chat) + Services (airtime, data, bills)",
      )
      .catch(() => {});
    bot.api
      .setMyDescription(
    ...
    bot.command("start", async (ctx) => {
      ...
      // Set menu button so Mini App is always one tap away
      await ctx.api.setChatMenuButton(ctx.chat.id, {
        type: "web_app",
        text: "SwiftyEx",
    ...
    if (!publicUrl.startsWith("https")) {
      console.warn(
        `⚠ Mini App URL is HTTP (${publicUrl}). Telegram requires HTTPS. Use ngrok or deploy to test.`,
      );
    }
